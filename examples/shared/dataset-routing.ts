 ...
 */
export function tryKilnViewerHref(
  input: string,
...
/** Local load on the unified viewer (`?local=true` for Zarr, `?local=ims` for Imaris). */
export function localViewerHref(opts: {
  baseUrl: string;
  currentPathname: string;
  kind?: "zarr" | "ims";
}): string {
  const siteRoot = viewerSiteRoot(opts.baseUrl);
  return opts.kind === "ims"
    ? `${siteRoot}app/?local=ims`
    : `${siteRoot}app/?local=true`;
}
