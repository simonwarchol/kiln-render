...
export function mountDatasetDialog(opts: DatasetDialogOptions): void {
  ...
  dialog.innerHTML = `
    ...
    <div class="dialog-content">
      ...
      <div class="load-option">
        <div class="load-option-title">${FILE_ICON_SVG}Local Imaris File</div>
        <div class="load-option-desc">Select an .ims file from your computer (Chrome/Edge only)</div>
        <button id="local-ims-btn" class="load-btn">
      ...
      <div class="load-option">
        ...
        <div class="remote-input-group">
          <input type="text" id="remote-url-input" class="remote-input" placeholder="https://example.com/dataset.ome.zarr or .ims">
  ...
  const localImsBtn = document.getElementById(
    "local-ims-btn",
  ) as HTMLButtonElement | null;
  const remoteInput = document.getElementById(
    "remote-url-input",
  ) as HTMLInputElement | null;
  const remoteLoadBtn = document.getElementById(
    "remote-load-btn",
  ) as HTMLButtonElement | null;
    ...
    if (!("showDirectoryPicker" in window)) {
      ...
      localBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          showDialogError([
            "Could not read dataset metadata — is this a valid .zarr directory?",
          ]);
          await clearHandle();
          return;
        } finally {
  ...
  if (localImsBtn) {
    if (!("showOpenFilePicker" in window)) {
      localImsBtn.disabled = true;
      localImsBtn.textContent = "Not supported in this browser";
    } else {
      localImsBtn.addEventListener("click", async () => {
        clearError();
        let handle: FileSystemFileHandle;
        try {
          handle = await promptForImsFile();
        } catch (e) {
          const msg = e instanceof Error ? e.message : "";
          if (!msg.includes("cancelled") && !msg.includes("aborted")) {
            showDialogError([msg || "Failed to open file"]);
          }
          return;
        }

        const orig = localImsBtn.textContent ?? "";
        localImsBtn.disabled = true;
        localImsBtn.textContent = "Checking…";
        try {
          const file = await handle.getFile();
          const reasons = await preValidateLocalIms(file);
          if (reasons.length > 0) {
            await clearHandle();
            showDialogError(reasons);
            return;
          ...
          showDialogError([
            "Could not read Imaris file — is this a valid .ims?",
          ]);
          await clearHandle();
          return;
        } finally {
          localImsBtn.disabled = false;
          localImsBtn.textContent = orig;
        }
    ...
    const loadRemote = async () => {
      ...
      try {
        if (looksLikeImsUrl(url)) {
          ...
          showDialogError([
            "No Zarr metadata found at this URL (expected zarr.json or .zgroup)",
          ]);
          return;
        }
      } catch (_) {
        showDialogError([
          "Could not reach dataset — check the URL is correct and publicly accessible",
        ]);
        return;
      } finally {
