...
describe("viewerSiteRoot", () => {
  ...
  it("strips legacy /app/multichannel/ suffix", () => {
    expect(viewerSiteRoot("/kiln-render/app/multichannel/")).toBe(
      "/kiln-render/",
    );
...
describe("tryKilnViewerHref", () => {
  ...
  it("recognises a basic viewer share link", () => {
    const input =
      "https://kilnrender.com/app/?dataset=https://example.com/vol.ome.zarr&mode=dvr";
    expect(tryKilnViewerHref(input, origin)).toBe(
  ...
  it("normalises cross-origin kiln links onto /app/", () => {
    const input =
      "https://kilnrender.com/app/multichannel/?dataset=https://example.com/x.zarr";
  ...
  it("rejects plain zarr dataset URLs", () => {
    expect(
      tryKilnViewerHref(
        "https://uk1s3.embassy.ebi.ac.uk/bia-integrator-data/S-BSST410/IM4/IM4.zarr/0",
        origin,
      ),
    ).toBeNull();
  });
...
describe("localViewerHref", () => {
  ...
  it("loads local Imaris on /app/?local=ims", () => {
    expect(
      localViewerHref({
        baseUrl: "/app/",
        currentPathname: "/app/",
...
describe("countChannels", () => {
  ...
  it("handles v0.5 typed axes", () => {
    ...
    expect(
      countChannels({ datasets: [{ path: "0" }], axes }, [2, 10, 10, 10]),
    ).toBe(2);
  });
});
